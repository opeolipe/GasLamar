...
export async function handleSessionPing(request, env) {
  ...
  if (!session_id) {
    return withRl(jsonResponse({ ok: false, expired: true }, 401, request, env));
  }

  const session = await getSession(env, session_id);
  if (!session) {
    return withRl(jsonResponse({ ok: false, expired: true }, 404, request, env));
  }

  // Re-write to refresh KV TTL while user is still active on the page
  await updateSession(env, session_id, { last_active: Date.now() });

  return withRl(jsonResponse({ ok: true, status: session.status }, 200, request, env));
}
