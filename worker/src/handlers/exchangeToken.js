...
export async function handleExchangeToken(request, env) {
  ...
  if (!email_token || typeof email_token !== 'string' || email_token.length > 64) {
    return jsonResponse({ message: 'Token tidak valid' }, 400, request, env);
  }

  // Sanitize: only allow hex characters (tokens are generated by hexToken())
  if (!/^[0-9a-f]+$/.test(email_token)) {
    return jsonResponse({ message: 'Token tidak valid' }, 400, request, env);
  }
