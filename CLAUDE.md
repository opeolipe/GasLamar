...
| `worker/src/handlers/mayarWebhook.js` | HMAC-SHA256 verification + idempotency sentinel `payment_processed_<session_id>` (48h TTL) — written BEFORE session update to survive retries. Normalizes many Mayar status variants (paid/settlement/capture/SUCCESS/…). Email send via `ctx.waitUntil`. **Webhook debugging:** if `webhook_no_session` appears in logs, compare `triedIds` against KV. `/create-payment` writes TWO indexes per invoice: `mayar_session_{invoice_id}` AND `mayar_session_{transaction_id}` — Mayar's webhook sends `data.id = transactionId` (a different UUID from the invoice ID). Both must exist in KV for the lookup to succeed. See `docs/payment-flow.md` → KV Keys. |
