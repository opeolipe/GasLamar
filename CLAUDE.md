...
- **CV truncated in tailoring** — `tailoring.js` activates section-aware truncation for CVs over 4000 chars: keeps header + first 2 experience entries (role-separator-based) + skills section, then appends a localized `[... removed ...]` note. Hard fallback: if section parsing yields no reduction AND the CV exceeds 10 000 chars, it is hard-cut at 10 000 with a note. Both paths emit a `console.warn` log (`cv_truncated` / `cv_truncated_hard`). CVs under 4000 chars are passed through unchanged.
