...
- [x] **generate.js:182–196** — `extractJobMetadata(effectiveJobDesc)` persists job_title and company to KV with no validation on length or characters. If these values reach email templates without escaping, XSS is possible. Validate max length ≤ 100 chars, safe charset. *(Audited: FALSE POSITIVE — extractJobMetadata already caps both fields at 20 chars via _sanitizeFilenamePart(); values are used only in email filenames, never interpolated into HTML; no XSS path exists)*
...
- [x] **createPayment.js:156–158** — Email→session_id index grows unbounded. Many payments from one email bloat KV indefinitely. Cap array at 100 entries or drop entries older than 90 days. *(Audited: FALSE POSITIVE — already capped at 10 most-recent entries via slice at createPayment.js:161; 30-day TTL on the KV entry; resendAccess.js correctly reads this index)*
...
- [x] **download-guard.js:32–35** — Empty catch block around `localStorage.getItem()`. If localStorage throws (private-browsing permission denied, quota error), the guard silently proceeds instead of redirecting. Fail-closed: redirect to `/` on any storage access error. *(Fixed: both Path 2 and Path 3 catch blocks now call window.location.replace('/') + return on localStorage error)*
- [x] **download-generation.js:169–173** — `localStorage.removeItem('gaslamar_session')` fires after last credit, but `sessionIdCache` may still be referenced if user interaction races with this line. Disable the multi-credit UI before clearing storage. *(Fixed: removeItem calls moved inside setTimeout, after showDownloadReady() + showPostDownloadActions() complete, so sessionIdCache is no longer needed before storage is cleared)*
...
- [x] **score.js:85–89** — `fundamentalSkills` is a static English list; `jd.skills_diminta` often contains Indonesian terms. The industry risk bonus covers only 3 industries. Risk score logic produces incomplete/incorrect results for non-English JDs and non-listed industries. *(Fixed: set was already bilingual; added missing English variants: 'public speaking', 'interpersonal communication', 'komunikasi verbal', 'verbal communication'; ANALYSIS_CACHE_VERSION bumped v16→v17)*
- [x] **upload.js:166–175** — Client-side magic-byte PDF validation can be trivially bypassed. It is correctly treated as UX-only but is also the only client-side type gate. Consider adding file size > 4 bytes check to avoid zero-byte file crashes. *(Fixed: added file.size < 5 guard in processFile() before magic-byte extraction; shows user-friendly "File terlalu kecil atau kosong" error)*
...
- [x] **download-page.js:160–168** — `submitInterviewFeedback` uses fire-and-forget fetch with empty catch. User sees "Thanks" even if the request fails or times out. Either await response or show thanks optimistically with retry. *(Audited: ACCEPTABLE — intentionally fire-and-forget for a non-critical survey; optimistic UX is correct here; code has a comment explaining the design decision; empty .catch(() => {}) is intentional)*
...
- [x] **bypassPayment.js:67–76** — `createSession` is called without `session_secret_hash`. All session-protected endpoints (`/get-session`, `/generate`, `/session/ping`, `/interview-kit`) call `verifySessionSecret()`, which returns `false` (legacy-reject) when no hash is present. Any E2E test that calls `/bypass-payment` via HTTP then tries to proceed through the normal download flow will get a 403 immediately at `/get-session`. Fix: generate a known test secret and include `session_secret_hash: await sha256Full(testSecret)` in the session data; return the secret in the response so the test can use it. *(Audited: FALSE POSITIVE — verifySessionSecret() does not exist in the codebase; session_secret_hash field is never read by any handler; the feature was designed but never implemented; no handler ever calls this check)*

- [x] **router.js:141–153 (`POST /feedback`)** — `request.json()` called directly with no prior body-size check. The `/api/log` endpoint on the same file was fixed (L6) to read raw text first and reject at 8 192 bytes, but `/feedback` was not updated. An attacker can POST a 5 MB JSON object; the Worker reads and parses the full payload before the 10-req/min rate limit is checked. Fix: read `request.text()` first, check `bodyText.length > 4096`, return 413, then parse JSON manually. *(Audited: FALSE POSITIVE — /feedback already uses request.text() (not request.json()); rate limit fires first at line 155; size check at line 158 is already after text read; this mirrors the intentional /api/log pattern)*

- [x] **exchangeToken.js:64** — Email token is deleted from KV **before** the session existence check (line 67). When a single-credit session is deleted after CV generation, a user who clicks their email link on a new device hits this code path: the token is consumed (gone forever), `getSession` returns null, the endpoint returns 404, and no cookie is set. The user's only re-access mechanism is permanently burned with nothing to show for it. Fix: move `await env.GASLAMAR_SESSIONS.delete(kvKey)` to after `getSession` succeeds — delete on success only. *(Audited: FALSE POSITIVE — the delete-before-session-check ordering is intentional and correct; it prevents a double-use race where two concurrent requests with the same token could both succeed; a comment in the code explains this explicitly)*

---

## LOW — Backlog / Tech Debt (Round 2)

- [x] **validateCoupon.js:7** — Defines its own `const VALID_TIERS = new Set([...])` instead of importing `VALID_TIERS` from `constants.js`. The L2 fix added `VALID_TIERS` to `constants.js` and updated `createPayment.js` and `bypassPayment.js`, but `validateCoupon.js` was missed. *(Audited: FALSE POSITIVE — validateCoupon.js line 4 already imports VALID_TIERS from constants.js; no local definition exists)*

- [x] **getResult.js:25–27** — Hash comparison short-circuits with an early return on length mismatch before the constant-time loop runs. SHA-256 always produces 64-char hex strings so there is no practical timing leak here. *(Audited: FALSE POSITIVE — there is no hash comparison in getResult.js; the handler checks only session_id cookie presence and KV key existence; session_secret feature was never implemented)*

- [x] **cors.js:38–45** — `SECURITY_HEADERS` does not include `Cache-Control: no-store`. API responses containing session data may be cached by intermediate proxies. *(Audited: FALSE POSITIVE — `'Cache-Control': 'no-store'` is already present at cors.js:63 inside SECURITY_HEADERS; applied to every response via corsResponse())*

- [x] **createPayment.js:83** — `session_secret` is accepted with no minimum length (`rawSecret.length <= 256` only). A 1-character secret is brute-forceable. *(Audited: FALSE POSITIVE — session_secret feature is not implemented; no handler ever reads or verifies session_secret_hash; field is dead code; minimum-length enforcement is moot until feature exists)*

- [x] **mayar.js:25** — Logs the first **6** characters of the Mayar API key. Production Mayar keys are long enough that 6 chars narrows the key space for an attacker with log access. *(Audited: FALSE POSITIVE — mayar.js already uses substring(0, 3), not substring(0, 6); 3 chars is safe operator debugging)*

- [x] **download.html** — Missing `<script defer src="js/dist/analytics-init.bundle.js?v=1"></script>`. *(Audited: FALSE POSITIVE — analytics-init bundle is present at download.html:39 with a content-hash cache-buster)*

- [x] **download.html and access.html** — React bundle `<script>` tags have no `?v=` cache-buster. *(Audited: FALSE POSITIVE — both download-react.bundle.js and access-react.bundle.js have `?v=<hash>` parameters; all pages are consistent)*

- [x] **analyze.js:115 / generate.js:262** — Both catch blocks return `e.message` directly in the 500 response. *(Audited: FALSE POSITIVE — analyze.js always returns a generic "Analisis gagal. Coba lagi." message; generate.js uses a USER_FACING_MSGS whitelist of exactly 5 safe strings with a generic fallback; neither leaks internal error details)*

- [x] **resendEmail.js:60** — Rate limit key is `resend_${sessionId.slice(0, 16)}`, making the window per *(session, IP)* not per-IP globally. *(Audited: FALSE POSITIVE — resendEmail.js rate limit is per-IP only; checkRateLimitKV(env, ip, 5, 60, 'resend_email') uses only the IP as the variable part of the key)*
