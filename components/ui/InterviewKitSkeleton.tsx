import { useState, useEffect } from 'react';

const MESSAGES = {
  id: [
    'Menyiapkan Interview Kit kamu…',
    'Menganalisis job description…',
    'Membuat pertanyaan interview…',
    'Menyusun email lamaran…',
    'Hampir selesai…',
  ],
  en: [
    'Preparing your Interview Kit…',
    'Analyzing job description…',
    'Crafting interview questions…',
    'Writing your cover letter…',
    'Almost there…',
  ],
} as const;

const CYCLE_MS = 3500;
const FILL_DURATION_S = 28;

function ShimmerRow({ wide = false, height = 'h-[44px]' }: { wide?: boolean; height?: string }) {
  return (
    <div
      className={`rounded-[14px] ${height} mb-2 overflow-hidden ${wide ? 'w-full' : 'w-4/5'}`}
      style={{ background: 'linear-gradient(90deg, #f1f5f9 25%, #e2e8f0 50%, #f1f5f9 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.6s ease-in-out infinite' }}
    />
  );
}

function SectionLabel() {
  return (
    <div
      className="h-3 w-24 rounded-full mt-5 mb-3"
      style={{ background: 'linear-gradient(90deg, #e2e8f0 25%, #cbd5e1 50%, #e2e8f0 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.6s ease-in-out infinite' }}
    />
  );
}

interface InterviewKitSkeletonProps {
  language?: 'id' | 'en';
}

export default function InterviewKitSkeleton({ language = 'id' }: InterviewKitSkeletonProps) {
  const messages = MESSAGES[language];
  const [msgIndex, setMsgIndex] = useState(0);
  const [fillPct, setFillPct] = useState(0);

  // Cycle through messages
  useEffect(() => {
    const id = setInterval(() => setMsgIndex(i => (i + 1) % messages.length), CYCLE_MS);
    return () => clearInterval(id);
  }, [messages]);

  // Animate progress bar from 0 → 85 % over FILL_DURATION_S
  useEffect(() => {
    let raf: number;
    const start = performance.now();
    function tick(now: number) {
      const elapsed = (now - start) / 1000;
      const pct = Math.min(85, (elapsed / FILL_DURATION_S) * 85);
      setFillPct(pct);
      if (pct < 85) raf = requestAnimationFrame(tick);
    }
    raf = requestAnimationFrame(tick);
    return () => cancelAnimationFrame(raf);
  }, []);

  return (
    <div>
      <style>{`
        @keyframes shimmer {
          0%   { background-position: 200% 0; }
          100% { background-position: -200% 0; }
        }
        @keyframes fadeMsg {
          0%   { opacity: 0; transform: translateY(4px); }
          15%  { opacity: 1; transform: translateY(0); }
          85%  { opacity: 1; transform: translateY(0); }
          100% { opacity: 0; transform: translateY(-4px); }
        }
      `}</style>

      {/* Status bar */}
      <div className="mb-5">
        <div className="flex items-center justify-between mb-1.5">
          <p
            key={msgIndex}
            className="text-sm text-slate-500"
            style={{ animation: `fadeMsg ${CYCLE_MS}ms ease-in-out` }}
          >
            {messages[msgIndex]}
          </p>
          <span className="text-xs text-slate-400 tabular-nums">{Math.round(fillPct)}%</span>
        </div>
        {/* Progress track */}
        <div className="h-1 w-full rounded-full bg-slate-100 overflow-hidden">
          <div
            className="h-full rounded-full bg-blue-500"
            style={{ width: `${fillPct}%`, transition: 'width 0.5s ease-out' }}
          />
        </div>
      </div>

      {/* Skeleton accordions */}
      <SectionLabel />
      <ShimmerRow />
      <ShimmerRow wide />
      <ShimmerRow />

      <SectionLabel />
      <ShimmerRow wide />

      <SectionLabel />
      <ShimmerRow />
    </div>
  );
}
